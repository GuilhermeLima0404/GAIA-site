...
def main(page: ft.page):
    ...
    def adicionar_cor(e):
        ...
        if erro:
            ...
            # PROXIMO DIA ?
            if int(Text_field_fecha[:2]) > int(Text_field_abre[
                                               :2]):  # SE A HORA DE FECHAR FOR DEPOIS DE ABRIR E SE FOREM IGUAIS TEREMOS QUE VERIFICAR OS MINUTOS
                page.open(msg_proximo_dia)  # SE O MINUTO DA HORA DE FECHAR FOR MAIOR DA DE ABRIR = proximo_dia()
            elif int(Text_field_fecha[:2]) == int(Text_field_abre[:2]) and int(Text_field_fecha[3:]) > int(
                    Text_field_abre[3:]):
                print(Text_field_fecha[:2], Text_field_abre[:2])
                page.open(msg_proximo_dia)
            else:
                        ...
                        ft.Container(bgcolor=verde1, height=60, border_radius=10, offset=ft.transform.Offset(-0.05, 0),
                                     content=ft.Row(width=100,
                                                    controls=[
                                                        ft.Text(
                                                            offset=ft.transform.Offset(0.03, 0),
                                                            value=f'Fechará as {horarios_cortinas_semana['Dom'][p]['fecha']}, abrirá as {horarios_cortinas_semana['Dom'][p]['abre']}' if len(
                                                                horarios_cortinas_semana['Dom'][p][
                                                                    'abre']) < 6 else f'Fechará as {horarios_cortinas_semana['Dom'][p]['fecha']}, abrirá as {horarios_cortinas_semana['Dom'][p]['abre'][:5]}' + 'H' + f'{horarios_cortinas_semana['Dom'][p]['abre'][5:]}',
                                                            color=branco2,
                                                            size=12.5,
                                                        ),
                                                        ft.IconButton(icon=ft.icons.REMOVE, on_click=Remove,
    ...
    Mostradores = ft.RadioGroup(on_change=FoodChange, content=ft.Column(
        controls=[ft.Radio(value="Pequeno", label="Pequeno", label_style=ft.TextStyle(color=verde2), fill_color=verde2),
                  ft.Radio(value="medio", label="Médio", label_style=ft.TextStyle(color=verde2), fill_color=verde2),
                  ft.Radio(value="Grande", label="Grande", fill_color=verde2, label_style=ft.TextStyle(color=verde2))]))
    ...
    botao_fechar_exemplos = ft.TextButton(text='Fechar', icon=ft.cupertino_icons.XMARK, icon_color=branco2,
    ...
    icon_aquecer = ft.Icon(name=ft.cupertino_icons.SUN_MAX, color=branco2, size=50, visible=True,
    ...
    IconTemp = ft.ElevatedButton(icon=ft.cupertino_icons.THERMOMETER_SUN, text='Temperatura', color=branco2,
                                 bgcolor='#ff8800', offset=ft.transform.Offset(0, 0.2))
    IconUmid = ft.ElevatedButton(icon=ft.cupertino_icons.CLOUD_RAIN, text='Umidade', color=branco2, bgcolor='#0096c7',
    ...
    page.navigation_bar = ft.CupertinoNavigationBar(
        ...
        destinations=[
            ft.NavigationBarDestination(icon=ft.icons.FOOD_BANK_SHARP, label="Alimentação"),
            ft.NavigationBarDestination(icon=ft.cupertino_icons.CLOUD, label="Clima"),
            ft.NavigationBarDestination(icon=ft.cupertino_icons.CAMERA, label="Monitoramento"),
            ft.NavigationBarDestination(icon=ft.icons.CURTAINS_ROUNDED, label="Cortinas"),
        ],
    )
